/**
 * LocalStorageMgr扩展类
 * 本类定位是解决LocalStorageMgr的使用过程中分别保存全局数据和帐号绑定数据分开保存的问题
 * 使用说明：
...
export class LocalStorageMgr {
    ...
    public static setHashID(openid: string): void {
        LocalStorageMgr.hashID = openid;
        LocalStorageMgr.setItem('hashID', openid, true);
    }
    ...
    public static getHashID(): string {
        return LocalStorageMgr.getItem('hashID', true);
    }
    ...
    public static setItem(key: string, value: string, isGlobal: boolean = false): void {
        if (isGlobal) {
            ...
            cc.sys.localStorage.setItem(`${LocalStorageMgr.hashID}_${key}`, value);
        }
    }
    ...
    public static getItem(key: string, isGlobal: boolean = false): string {
        if (isGlobal) {
            ...
            return cc.sys.localStorage.getItem(`${LocalStorageMgr.hashID}_${key}`);
        }
    }
    ...
    public static removeItem(key: string, isGlobal: boolean = false): void {
        if (isGlobal) {
            ...
            cc.sys.localStorage.removeItem(`${LocalStorageMgr.hashID}_${key}`);
        }
    }
